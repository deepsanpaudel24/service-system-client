...
import {
  AddEmployeeDispatcher,
  AddEmployeeResponseReset,
} from "../../actions/employee_management/AddEmployeeAction";
...
const AddEmployee = ({ t }) => {
  ...
  const confirmEmployeeRegister = () => {
    if (!_.isEmpty(response.data)) {
      ...
      return history.push("/sadmin/employees");
    }
  };
  ...
  return (
    <div class="flex mb-4">
      <div class="w-3/5 ml-5">
        <form>
          <p class="text-3xl my-3">{t("invite_employees")}</p>
          {showServerError()}
...
export default withTranslation()(AddEmployee);
