...
const TransferPayment = (props) => {
    const { spname, spid, clientname, clientid, casetitle, caseid, redirectAfterTransfer } = props
            ...
            customUI: ({ onClose }) => {
              return (
                <div className="bg-white shadow border rounded px-4 py-4 mx-auto max-w-lg">
                    <div class="flex px-4">
                        <h1 class="text-3xl text-blue-600 px-4">Send funds to {spname}</h1>
                    </div>
                    <hr class="border-gray-300 my-4" />
                    <div class= "px-4">
                        <div class={`items-center bg-blue-100 text-black px-4 py-3 mb-3`} role="alert">
                            <div class="flex">
                                <div class="py-1">
                                    <svg class="fill-current w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M12.432 0c1.34 0 2.01.912 2.01 1.957 0 1.305-1.164 2.512-2.679 2.512-1.269 0-2.009-.75-1.974-1.99C9.789 1.436 10.67 0 12.432 0zM8.309 20c-1.058 0-1.833-.652-1.093-3.524l1.214-5.092c.211-.814.246-1.141 0-1.141-.317 0-1.689.562-2.502 1.117l-.528-.88c2.572-2.186 5.531-3.467 6.801-3.467 1.057 0 1.233 1.273.705 3.23l-1.391 5.352c-.246.945-.141 1.271.106 1.271.317 0 1.357-.392 2.379-1.207l.6.814C12.098 19.02 9.365 20 8.309 20z"/></svg>
                                </div>
                                <div>
                                    <p class="font-bold text-md text-gray-800">You are about to pay {total_payable_amount} {sp_currency} stripe balance.</p>
                                    <p class="text-sm text-gray-800 my-1">
                                        <b>{total_payable_amount} {sp_currency}</b> from your stripe account is being paid to <b>{spname}</b> on behalf of <b>{clientname}</b> for the case - <b>{casetitle}</b>. 
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="items-center text-black px-4 py-3 mb-3" role="alert">
                            <div class="mb-2">
                                <p class="text-md font-bold text-gray-900">Your available stripe balance {admin_stripe_balance/100} {admin_stripe_currency}</p>
                            </div>
                            <div class="my-1">
                                <p class="text-sm text-gray-900">Total case fee ({sp_currency})</p>
                            </div>
                            <input 
                            ...
                            />
                            <div class="my-1">
                                <p class="text-sm text-gray-900">Application fee ({sp_currency})</p>
                            </div>
                            <input 
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                                id="file_title" 
                            ...
                            />
                            <div class="my-1">
                                <p class="text-sm text-gray-900">Amount to be paid ({sp_currency})</p>
                            </div>
                            <input 
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                                id="file_title" 
        ...
        <div>
            {
                ...
                :
                <button class="bg-blue-600 text-white px-3 py-2" onClick={() => OpenPaymentReleaseModal()}>Release Payment</button>
            }
        </div>
    )
}
