...
import { SetPeoplePasswordDispatcher } from "../actions/people_mangement/SetPeoplePasswordAction";

import { withTranslation } from "react-i18next";
import { useHistory } from "react-router";

const PeoplePasswordSetup = ({ t }) => {
  ...
  const response = useSelector((state) => state.SetPeoplePasswordResponse);
  ...
  const dataValidator = () => {
    if (!strongRegex.test(password)) {
      ...
      dispatch(SetPeoplePasswordDispatcher(data, token));
    }
  };
...
export default withTranslation()(PeoplePasswordSetup);
